...
// Money-tolerant: "$24,000" / "24,000" / 24000 all → 24000
function money(v) {
  if (v == null || v === '') return null;
...
function parseArrays(vrows, drows, sourceLabel) {
  ...
  // The OFFER columns are located by header name (with the snapshot's
  // positions as fallback) — the live sheet's layout can drift and rates
  // silently vanishing is exactly the failure we can't have.
  const mvHdrRow = (vrows[0] || []).map(c => String(c == null ? '' : c).trim().toLowerCase());
  const hdrIdx = (name, fallback) => {
    const i = mvHdrRow.indexOf(name);
    return i >= 0 ? i : fallback;
  };
  const CI_RATE_TA = hdrIdx('rate ta', 71);
  const CI_BB_TA = hdrIdx('bb ta', 72);
  const CI_RATE_FH = hdrIdx('rate fh', 73);
  const CI_BB_FH = hdrIdx('bb fh', 74);
  ...
  for (let ri = 1; ri < vrows.length; ri++) {
    ...
    vessels.push({
      ...
      // Offers: TA and FH rates + ballast bonuses where quoted — header-located
      // columns, money-tolerant parsing ("$24,000" and "24,000" both work)
      rate_ta: money(r[CI_RATE_TA]), bb_ta: money(r[CI_BB_TA]),
      rate_fh: money(r[CI_RATE_FH]), bb_fh: money(r[CI_BB_FH]),
    });
  }
  return {
