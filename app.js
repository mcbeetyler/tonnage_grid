...
const ALL_COLUMNS = ['laycan','vessel','owner','specs','draft','yard','origin','scr','delivery','laycan_date','eta','hire_offer','bb_offer','bki_eqvt','rate_pmt','arrow_eqvt','bunker','p6_bid','p6_offer','spread','fixed','charterer','date_fixed','last_updated','notes','status','actions'];
...
function parseAgeField(s) {
  ...
  if (!isNaN(n) && n < 100) return 2000 + n;
  return null;
}
...
function looksLikeCSV(raw) {
  ...
  const cells = splitCSVRow(firstLine, delim).map(c => normalizeHeader(c));
  // Need at least a vessel column + 2 other recognisable columns
...
function parseCSVVessels(raw) {
  ...
  const headerCells = splitCSVRow(lines[0], delim).map(normalizeHeader);
  const colMap = headerCells.map(h => CSV_HEADER_MAP[h] || null);

  const vessels = [];
  for (let i = 1; i < lines.length; i++) {
    const cells = splitCSVRow(lines[i], delim).map(c => (c || '').trim());
    // Skip empty rows
    ...
    for (let j = 0; j < cells.length; j++) {
      ...
      switch (field) {
        case 'vessel_name':
          // strip trailing "75Dwt"-style suffix so we keep just the name
        ...
        case 'laycan_date': {
          v.laycan_date = val;
          // Also set open_date if parseable
    ...
    // Populate market_colour from hire_offer so P6 offer / spread can flow
...
async function handleParse() {
  ...
  if (looksLikeCSV(raw)) {
    try {
      const parsed = parseCSVVessels(raw);
      if (parsed.length === 0) {
