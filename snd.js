...
function mapSvg(grid, side, land) {
  ...
  return `<svg class="sd-map" viewBox="0 0 ${MAP.w} ${MAP.h}" preserveAspectRatio="xMidYMid meet">` +
    `<rect class="sd-sea" width="${MAP.w}" height="${MAP.h}"/>${landPath}${polys.join('')}${labels.join('')}</svg>`;
}
...
function renderMaps(grid) {
  ...
  el.innerHTML = `
    ...
    <div class="sd-mapcard" data-side="demand"><div class="sd-maptitle">Cargo <span class="sd-th-sub">live cargoes vs run rate · bar = fronthaul vs TA · hover for detail</span></div>${mapSvg(grid, 'demand', land)}<div class="sd-tip" hidden></div></div>`;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    _test: {
      ...
      ZONE_POLYS, project, decodeTopo, heat, legLabel,
