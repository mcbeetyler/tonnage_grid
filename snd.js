...
const ui = Object.assign({ horizon: '30', lookback: '28', detail: true, map: true },
...
// Where a cargo is going, as a point to draw a flow to
const DEST_POINTS = {
  'Far East': [122, 30], 'India/PG': [66, 20], 'SE Asia/Oz': [135, -25], 'S Africa': [28, -31],
  'Med': [14, 38], 'Cont': [3, 54], 'Americas': [-70, 12], 'W Africa': [2, 3],
};
// Atlantic viewport, 4 px per degree; destinations beyond it (Far East,
// India) are drawn to the edge with a label
...
function mapSvg(grid, side, land) {
  ...
  for (const r of grid.rows) {
    ...
    for (const z of zones) {
      ...
      const zm = fillFor(z);
      polys.push(`<path class="sd-zone sd-fill-${zm.verdict.tone}" style="fill-opacity:${heat(zm).toFixed(2)}" d="${pathOf(poly)}" data-basin="${esc(r.basin)}"><title>${esc(ZONE_LABEL[z] || z)}: ${zm.now}${changeText(zm) ? ' ' + changeText(zm) : ''}</title></path>`);
    }
    ...
    labels.push(`<g class="sd-lbl" transform="translate(${cx.toFixed(1)},${cy.toFixed(1)})" data-basin="${esc(r.basin)}">` +
      `<rect x="-30" y="-14" width="60" height="28" rx="4"/>` +
      `<text y="-2" class="sd-lbl-n">${m.now}</text><text y="10" class="sd-lbl-c sd-fill-${m.verdict.tone}">${esc(ch || m.verdict.word)}</text></g>`);
    if (side === 'demand') {
      for (const d of (m.dims.dest || [])) {
        const to = DEST_POINTS[d.value] || (() => { const z = Object.keys(ZONE_LABEL).find(k => ZONE_LABEL[k] === d.value); return z && ZONE_POLYS[z] ? centroid(ZONE_POLYS[z]) : null; })();
        if (!to || !d.now) continue;
        let [tx, ty] = project(to[0], to[1]);
        const off = tx < 0 || tx > MAP.w || ty < 0 || ty > MAP.h;
        if (off) {   // clamp along the line to the viewport edge
          const dx = tx - cx, dy = ty - cy;
          let k = 1;
          if (tx > MAP.w) k = Math.min(k, (MAP.w - 6 - cx) / dx);
          if (tx < 0) k = Math.min(k, (6 - cx) / dx);
          if (ty > MAP.h) k = Math.min(k, (MAP.h - 6 - cy) / dy);
          if (ty < 0) k = Math.min(k, (6 - cy) / dy);
          tx = cx + dx * k; ty = cy + dy * k;
        }
        const mx = (cx + tx) / 2, my = Math.min(cy, ty) - Math.abs(tx - cx) * 0.18;
        flows.push(`<path class="sd-flow" style="stroke-width:${(0.8 + Math.min(d.now, 8) * 0.55).toFixed(1)}" d="M${cx.toFixed(1)} ${cy.toFixed(1)} Q${mx.toFixed(1)} ${my.toFixed(1)} ${tx.toFixed(1)} ${ty.toFixed(1)}"><title>${esc(r.basin)} → ${esc(d.value)}: ${d.now}</title></path>` +
          `<circle class="sd-flow-end" cx="${tx.toFixed(1)}" cy="${ty.toFixed(1)}" r="2.2"/>` +
          (off ? `<text class="sd-flow-lbl" x="${(tx - 4).toFixed(1)}" y="${(ty - 4).toFixed(1)}">${esc(d.value)} ${d.now}</text>` : ''));
      }
    }
  }
  return `<svg class="sd-map" viewBox="0 0 ${MAP.w} ${MAP.h}" preserveAspectRatio="xMidYMid meet">` +
    `<rect class="sd-sea" width="${MAP.w}" height="${MAP.h}"/>${landPath}${polys.join('')}${flows.join('')}${labels.join('')}</svg>`;
}
...
function renderMaps(grid) {
  ...
  const land = SD.land || [];
  el.innerHTML = `
    <div class="sd-mapcard"><div class="sd-maptitle">Tonnage <span class="sd-th-sub">open ships vs run rate · blue deep, amber thin</span></div>${mapSvg(grid, 'supply', land)}</div>
    <div class="sd-mapcard"><div class="sd-maptitle">Cargo <span class="sd-th-sub">live cargoes vs run rate · lines = where they're going</span></div>${mapSvg(grid, 'demand', land)}</div>`;
  el.querySelectorAll('[data-basin]').forEach(n => n.addEventListener('click', () => {
...
function injectCss() {
  ...
  st.textContent = `
    ...
    .sd-flow-lbl{font-family:var(--mono);font-size:8px;font-weight:600;fill:var(--accent);text-anchor:end}
    text.sd-fill-up{fill:var(--accent)} text.sd-fill-down{fill:var(--amber)} text.sd-fill-flat,text.sd-fill-dim{fill:var(--text-dim)}
    .sd-flow{fill:none;stroke:var(--accent);stroke-opacity:.55;stroke-linecap:round}
    .sd-flow-end{fill:var(--accent);fill-opacity:.8}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    _test: {
      ...
      ZONE_POLYS, DEST_POINTS, project, decodeTopo, heat,
