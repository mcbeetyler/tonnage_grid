...
// Expected column layout from Google Sheet (0-indexed):
// 0: charterer  1: size  2: load  3: disch  4: cargo  5: laycan
// 6: com  7: updated  8: comment  9: FRESH  10: FIXED  11: USER
const EXPECTED_COL_HEADERS = ['charterer','size','load','disch','cargo','laycan','com','updated','comment','fresh','fixed','user'];

function isHeaderRow(cells) {
  const joined = cells.map(c => (c || '').toLowerCase().trim()).join(' ');
  return /\bcharterer\b/.test(joined) && /\blaycan\b/.test(joined);
}
...
function parseCargoData(text) {
  ...
  let columnMap = null; // detected column positions {charterer: 0, size: 1, ...}
  ...
  for (const line of lines) {
    ...
    // Check if this is a stem header row (first cell is a known stem)
    const firstCell = trimmed[0].toLowerCase().replace(/\s+/g, ' ').trim();
    if (STEM_MAP[firstCell]) {
      currentStem = STEM_MAP[firstCell];
      columnMap = null; // reset — next row should be the header
      continue;
    }

    // Check if this is the column header row ("charterer size load disch ...")
    if (isHeaderRow(trimmed)) {
      columnMap = {};
      trimmed.forEach((cell, i) => {
        const key = cell.toLowerCase().trim();
        if (EXPECTED_COL_HEADERS.includes(key)) {
          columnMap[key] = i;
        }
      });
      continue;
    ...
    // Parse as cargo row using detected column map, falling back to default positions
    const map = columnMap || {
      charterer: 0, size: 1, load: 2, disch: 3, cargo: 4,
      laycan: 5, com: 6, updated: 7, comment: 8, fresh: 9, fixed: 10
    };
    ...
    // Skip if the "charterer" cell looks like a count or summary row
    if (/^\d+$/.test(charterer) || /^total/i.test(charterer)) continue;
    ...
    // Need a laycan OR cargo type to count as a valid row
...
The input is tab-separated text from a Google Sheet. It contains stem header rows (like "ECSA fronthaul", "USG TA") followed by column header rows ("charterer size load disch cargo laycan ..."), then cargo rows, then another stem header, etc.
...
- Bsea/Med Fronthaul, Bsea/Med TA

CARGO ROW FIELDS:
- charterer: name of charterer (strip "- NOT FOR LIST" suffix silently)
- size: vessel size like "pmx", "kmx", "66000", "pmx/kmx"
- load: load area like "ecsa", "ncsa", "usg", "santos"
- disch: discharge area like "china", "spore-jpn", "feast"
- cargo: cargo type like "grain", "coal", "ore", "petcoke"
- laycan: laycan string like "10apr onw", "15-30apr", "1-5may"
- updated: date the cargo was last updated (like "25Mar", "2Apr")
- fresh: boolean — true if FRESH column is TRUE
- fixed: boolean — true if FIXED column is TRUE
- stem: which stem section this cargo was under (e.g. "ECSA Fronthaul")

RULES:
- Stem header rows have just the stem name in the first cell (and maybe a count). Track the current stem — apply it to all following cargo rows until the next stem header.
- Skip the column header rows ("charterer size load disch ..."), total rows, and empty rows.
- If a cell is empty, use empty string "".
- Include every cargo row with a valid charterer.

Return ONLY a JSON array of cargo objects. No markdown, no explanation.`;
