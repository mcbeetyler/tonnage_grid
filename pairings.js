...
function buildPairingsText() {
  ...
  if (!w) return null;
  const px = r => isTa ? r.hireTa : r.p6offer;
  // FIT only, with a rate — EARLY and TIGHT stay off the principal's list
  const picks = rows.filter(r => r.status === 'FIT' && !r.reason && px(r) != null);
  ...
  for (const r of picks) {
    ...
    let line1 = `*${v.vessel_name || '?'} ${specs}${v.scrubber === true ? ' SCR' : ''}*`;
  ...
  root.innerHTML = `
    <div style="padding:20px 28px">
      ...
      <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:6px">
        ...
        <button id="pr_copy" title="Copy a WhatsApp-ready tonnage list: FIT ships only (no early/tight) WITH an offer. ETAs at the load area, no internal comments."
